...
$sentencia = $base_de_datos->query("SELECT ventas.total, ventas.fecha, ventas.id, GROUP_CONCAT(	productos.id, '..',  productos.descrip, '..',  productos.precio, '..', productos_vendidos.cantidad , '..', productos.nomProducto, '..', envio.garantia SEPARATOR '__') AS productos FROM ventas INNER JOIN productos_vendidos ON productos_vendidos.id_venta = ventas.id INNER JOIN productos ON productos.id = productos_vendidos.id_producto INNER JOIN envio ON envio.id = ventas.id GROUP BY ventas.id ORDER BY ventas.id;");
